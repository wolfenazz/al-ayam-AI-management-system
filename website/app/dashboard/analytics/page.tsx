...
export default function AnalyticsPage() {
    ...
    const avgResponseTime = Math.round(
        ...
        (mockTasks.filter(t => t.response_time).length || 1) / 60
    );

    // Task distribution by type
    const tasksByType = mockTasks.reduce((acc, t) => {
        acc[t.type] = (acc[t.type] || 0) + 1;
        return acc;
    }, {} as Record<string, number>);

    // Task distribution by status
    const tasksByStatus = mockTasks.reduce((acc, t) => {
        ...
        <div className="p-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    ...
                    <BigMetric icon="group" label="Active Team" value={mockEmployees.filter(e => e.status === 'ACTIVE').length.toString()} subtext={`${mockEmployees.filter(e => e.availability === 'AVAILABLE').length} currently available`} color="green" />
                </div>
